...
Vantage and Ink & Halo remain separate source folders; no suite-core registration
changes are included. Preview galleries and ZIPs are generated deliverables under
`artifacts`. Ink & Halo's focused checks are in `test_inkhalo.py`; its PNG previews
are in `artifacts/inkhalo/renders`.
