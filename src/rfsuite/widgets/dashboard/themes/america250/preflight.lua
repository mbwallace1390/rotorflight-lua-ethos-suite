...
local function preflightPaint(x, y, w, h, box, c, telemetry)
    ...
    local cy = bodyY + bodyH * (bodyH < 260 and 0.24 or 0.34)
    local radius = min(centerW * 0.33, bodyH * (bodyH < 260 and 0.15 or 0.26))
    ...
    lcd.color(C.panel)
    lcd.drawFilledRectangle(floor(cx - radius * 0.85), floor(cy - (bodyH < 260 and 22 or 46)), floor(radius * 1.70), bodyH < 260 and 66 or 104)
    drawTextAligned(centerX, cy - (bodyH < 260 and 22 or 46), centerW, c.status or "WAITING", bodyH < 260 and "FONT_L" or "FONT_XXL", C.white, "center")
    drawCenteredDotLine(centerX, cy + 7, centerW, "1776", "2026", "FONT_XS", C.amber)

    if c.issueText and bodyH >= 260 then
        ...
        drawTextAligned(centerX, cy + (bodyH < 260 and 24 or 37), centerW, c.statusSub or "AWAITING TELEMETRY", "FONT_XXS", c.statusColor or C.muted, "center")
    end
    ...
    if bodyH >= 260 then drawCenteredDotLine(
        centerX + 18,
        segY + 65,
        centerW - 36,
    ...
    drawCheckRow(rightX + 14, bodyY + cardH + pad + floor(cardH * 0.3), sideW - 28, "RATES", c.rateText or "--", C.red)
    drawCheckRow(rightX + 14, bodyY + cardH + pad + floor(cardH * 0.54), sideW - 28, "PID BANK", c.pidText or "--", C.white)
