...
local function updateFlightTime(cache)
    ...
    if cache._timerSecond ~= seconds or cache.timer == nil or cache._invalidTime ~= invalidTime then
        cache._invalidTime = invalidTime
        cache._timerSecond = seconds
        cache.timer = invalidTime and "--:--" or format("%02d:%02d", floor(seconds / 60), seconds % 60)
    end
end
...
local function drawThermalPlume(x, y, w, h, value, maximum, color, valueText)
    ...
    for i = 0, 7 do
        local bh = max(1, floor(max(0, h - 70) * pct * (0.48 + i / 15)))
        local bw = 5 + (i % 3) * 2
...
local function drawThrustArray(x, y, w, h, throttle, valueText, color)
    ...
    for i = 0, bars - 1 do
        local bh = max(2, (h - 70) * (0.24 + i * 0.08))
...
local function inflightPaint(x, y, w, h, box, c)
    ...
    lcd.color(C.panel)
    lcd.drawFilledRectangle(floor(cx - radius * 0.66), floor(cy - 52), floor(radius * 1.32), 107)
    drawTextAligned(cx - radius, cy - 52, radius * 2, c.rpmText or "--", "FONT_XXL", c.rpm and C.white or C.muted, "center")
    ...
    if bodyH >= 280 then drawTextAligned(cx - radius, cy + 52, radius * 2, "EVENT HORIZON", "FONT_XXS", C.violet, "center") end
